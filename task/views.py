...
def task_list(request):
  ...
  tasks = Task.objects.filter(by=request.user) | Task.objects.filter(
    team__memberships__member=request.user)
      ...
      try:
        ...
        for membership in memberships:
          ...
          try:
            ...
            extra_messages = {
              'error': f'There has been an error notifying other members. {e}'
            }
...
def update_task(request, id):
  ...
  if request.user.is_superuser or task.by == request.user or task.team and task.team.memberships.filter(
...
def delete_task(request, id):
  ...
  if task.by != request.user and not task.team.memberships.filter(
      member=request.user).exists():  #.user
    return Response({'error': 'Unauthorized.'},
                    status=status.HTTP_403_FORBIDDEN)
...
def create_team(request):
  ...
  if teamserializer.is_valid():
    ...
    this_new_team = Team.objects.get(
      teamname=teamserializer.validated_data['teamname'])
...
def add_user_to_team(request):
  ...
  return Response({'membership': 'membership created'},
                  status=status.HTTP_201_CREATED)
...
def leave_team(request, id):
  ...
  membership.delete()
  return Response({'message': 'Membership to team was cancelled.'},
